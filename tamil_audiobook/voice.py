...
# Exact text spoken in the selected 8.52-second bilingual segment extracted from the final accepted audiobook.
...
def _decode_default_opus() -> bytes:
    ...
    if hashlib.sha256(raw).hexdigest() != DEFAULT_VOICE_OPUS_SHA256:
        raise RuntimeError("packaged final accepted-C default voice checksum mismatch")
    return raw
